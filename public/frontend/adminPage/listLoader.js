...
modalConfirmBtn.onclick = async () => {
    ...
    try {
        const response = await fetch(`http://localhost:5001/api/users/${activeTargetUserId}/reject`, {
            method: 'PUT',
            headers: {
...
async function handleAction(userId, actionType) {
    try {
        const response = await fetch(`http://localhost:5001/api/users/${userId}/${actionType}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            }
...
async function fetchUserCount() {
    try {
        const response = await fetch('http://localhost:5001/api/user-count');
        const data = await response.json();
