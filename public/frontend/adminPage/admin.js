...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const confirmation_modal = document.getElementById('modal-cancel-btn');
  const confirmation = document.getElementById('confirmation-modal');
  confirmation_modal.addEventListener('click', () => {
...
window.addEventListener('storage', (event) => {
  if (event.key === 'tk_requests' || event.key === 'tk_roster') {
    loadData();
  }
});
...
function renderRequests() {
  ...
  if (pendingRequests.length === 0) {
    container.innerHTML =
      '<p style="text-align: center; font-size: 1.1rem; color: #888; padding: 2rem;">No pending registration requests.</p>';
    return;
  }
  ...
  pendingRequests.forEach((req) => {
    ...
    card.innerHTML = `
            ...
            <div class="request-details" style="margin: 0.8rem 0;">
                <p><strong>UID:</strong> ${req.uid}</p>
                <p><strong>Streamer Mode ID:</strong> ${req.streamerId}</p>
                <p><strong>FB Link:</strong> <a href="${req.fbLink}" target="_blank" rel="noopener">${req.fbLink}</a></p>
            </div>

            <div class="proof-images" style="display: flex; gap: 1rem; margin-bottom: 1rem;">
                <div class="proof-box">
                    <span style="display: block; font-size: 0.85rem; color: #aaa; margin-bottom: 0.3rem;">In-Game Profile</span>
                    ${req.gameProfileImg ? `<a href="${req.gameProfileImg}" target="_blank"><img src="${req.gameProfileImg}" alt="Game Profile" style="max-width: 120px; border-radius: 4px;"></a>` : 'No Image'}
                </div>
                <div class="proof-box">
                    <span style="display: block; font-size: 0.85rem; color: #aaa; margin-bottom: 0.3rem;">FB Profile</span>
                    ${req.fbProfileImg ? `<a href="${req.fbProfileImg}" target="_blank"><img src="${req.fbProfileImg}" alt="FB Profile" style="max-width: 120px; border-radius: 4px;"></a>` : 'No Image'}
                </div>
            </div>

            <div class="actions-row" style="display: flex; gap: 0.5rem;">
                <button class="btn btn-approve" onclick="openApprovalModal(${req.id})">Approve Member</button>
                <button class="btn btn-reject" onclick="rejectMember(${req.id})">Reject Request</button>
            </div>
        `;
